...
struct proxy_bd {
	...
	struct scatterlist sglist[1024*4];
};
struct dma_proxy_channel {
	struct dma_proxy_channel_interface *buffer_table_p;	/* user to kernel space interface */
...
{
	int bdindex = pchannel_p->bdindex;
	pchannel_p->bdtable[bdindex].dma_handle = (dma_addr_t)(pchannel_p->buffer_phys_addr + 
						(sizeof(struct dma_proxy_channel_interface) * bdindex) + offsetof(struct dma_proxy_channel_interface, buffer));
	
	enum dma_ctrl_flags flags = DMA_CTRL_ACK | DMA_PREP_INTERRUPT;
	struct dma_async_tx_descriptor *chan_desc;
	struct dma_device *dma_device = pchannel_p->channel_p->device;
	struct dma_proxy_channel_interface *interface_p = pchannel_p->buffer_table_p;

	chan_desc = dma_device->device_prep_dma_cyclic(pchannel_p->channel_p, pchannel_p->bdtable[bdindex].dma_handle, interface_p->length * RX_BUFFER_COUNT, interface_p->length, pchannel_p->direction, flags);
...
static long ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd) {
		case START_CYCLIC:
   		...
   		case STOP_CYCLIC:
            pchannel_p->channel_p->device->device_terminate_all(pchannel_p->channel_p);
            printk(KERN_INFO "[DMA ioctl] PROXY_DMA_CYCLIC_STOP\n");
        	break;

		case PROXY_DMA_SET_SIGNAL:{
			copy_from_user(&pchannel_p->sig_params, (signal_parameters*)arg, sizeof(pchannel_p->sig_params)); 
            printk(KERN_INFO "[DMA ioctl] PROXY_DMA_SET_SIGNAL period = %d\n", pchannel_p->sig_params.period);

            if(pchannel_p->sig_params.on)
			{
                pchannel_p->signal_pid = current->pid;
                pchannel_p->sig_per = pchannel_p->sig_params.period;
                pchannel_p->signal_data = 0xff & (pchannel_p->sig_params.data);
            } 
			else 
                pchannel_p->signal_pid = 0;

            pchannel_p->sinfo.si_signo = pchannel_p->sig_params.on;
...
static int create_channel(struct platform_device *pdev, struct dma_proxy_channel *pchannel_p, char *name, u32 direction)
{
	 ...
	 */
	pchannel_p->buffer_table_p = (struct dma_proxy_channel_interface *)
		dmam_alloc_coherent(pchannel_p->dma_device_p,
					sizeof(struct dma_proxy_channel_interface) * BUFFER_COUNT,
	...
	for (bd = 0; bd < BUFFER_COUNT; bd++) 
		pchannel_p->bdtable[bd].dma_handle = (dma_addr_t)(pchannel_p->buffer_phys_addr + 
						(sizeof(struct dma_proxy_channel_interface) * bd) + offsetof(struct dma_proxy_channel_interface, buffer));
